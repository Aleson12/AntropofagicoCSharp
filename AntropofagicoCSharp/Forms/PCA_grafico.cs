    ...
    public partial class PCA_grafico : Form
    {
        ...
        public void LocalizaPonto(double[] x, double[] y)
        {
            ...
            formsPlot1.MouseMove += (s, e) =>
            {
                ...
                List<DataPoint0> pontosDeDispersao = new List<DataPoint0>();
                ...
                // percorrendo cada arquivo.csv, obtendo apenas o seu nome e extensão:
                foreach (string nomeArquivoCSV in nomesDosArquivosCSV)
                {

                    DataPoint0 ponto = new DataPoint0
                    {
                        X = mousePixel.X,
                        Y = mousePixel.Y,
                        NomeDoCSV = nomeArquivoCSV
                    };
                    
                    pontosDeDispersao.Add(ponto);

                }

                foreach (DataPoint0 ponto in pontosDeDispersao)
                {

                    if (nearest.Index >= 0 && nearest.Index < pontosDeDispersao.Count)
                    {
                        ...
                        if (nearest.IsReal)
                            Text = $"Coordenadas: Y={nearest.X:0.##}, X={nearest.Y:0.##}; Origem: {nomesDosArquivosCSV[nearest.Index]}";
